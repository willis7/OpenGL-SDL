...
Files:      Main.cpp   (The Source File containing the most worked with code)
            ...
            Obj.cpp    (The Source File containing the OBJ loading code)
            Obj.h      (The header file that holds the class definations and
                        structures)
            foot.obj   (The 3D file that we load in)
            bone.bmp   (The texture map file for the foot object)

Libraries:  opengl and SDL 

Instructions:
        After unzipping the files, be in the 'OBJFileLoader' directory and for

        Compiling:
            1. Just type 'make' at the command prompt.
        Running:
            1. Just type './OBJLoader' at the command prompt.
